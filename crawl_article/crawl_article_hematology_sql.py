...
def  convert_to_pdf_direct(doi_url):
    if doi_url.startswith("https://ascopubs.org/doi/"):
        return doi_url.replace("/doi/", "/doi/pdfdirect/")
    else:
        return 'N/A'

async def  crawl_article_blood(specialization):
    journal_name =  'blood'
...
async def  crawl_article_clincal_oncology(specialization):
    ...
    try:
        ...
        for link_tuple in links:
            ...
            try:
                ...
                article_volume = soup.find('span', property='volumeNumber').getText()
                article_issue =  soup.find('span', property='issueNumber').getText()
                article_publication_date = meta_tags.get("dc.Date", "2001-01-01")
                article_publisher = meta_tags.get('dc.Publisher', 'N/A')
                article_full_pdf_link = convert_to_pdf_direct(article_url)
                ...
                if soup.find('section', property='abstract'):
                ...
                if abstract_section:
                    # If the abstract section exists, find all <p> tags within it
                    paragraphs = abstract_section.find_all('div')
                else:
                    # If the abstract section does not exist, find all <p> tags in the entire document
                    paragraphs = soup.find_all('p')
...
async def  crawl_article_haematologica(specialization):
    journal_name = 'Haematologica'
    connection = connection_config()
    table_name = specialization
...
async def  crawl_article_leukemia(specialization):
    journal_name = 'Leukemia'
    connection = connection_config()
    table_name = specialization
...
async def  crawl_article_blood_advances(specialization):
    journal_name = 'Blood Advances'
    connection = connection_config()
    table_name = specialization
