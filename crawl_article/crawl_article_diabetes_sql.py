...
async def crawl_article_diabetes(specialization):
    ...
    try:
        ...
        for link_tuple in links:
            ...
            try:
                ...
                paragraphs = []  # Initialize to an empty list
...
async def crawl_article_endocrine(specialization):
    journal_name = "Endocrine Reviews"
    connection = connection_config()
    table_name = specialization
            ...
            try:
                ...
                if len(paragraphs) >= 1:
                    article_abstract = " ".join(
                        x.get_text(strip=True) for x in paragraphs
                    )
                else:
...
async def crawl_article_diabetes_care(specialization):
...
async def crawl_article_dia(specialization):
...
async def crawl_article_jcem(specialization):
...
async def main():
    specialization = 'diabetes_endocrinology'
    await crawl_article_dia(specialization)
