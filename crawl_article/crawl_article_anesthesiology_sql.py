...
async def crawl_article_anesthesiology(specialization):
...
async def crawl_article_bja(specialization):
    ...
    try:
        ...
        for link_tuple in links:
            ...
            try:
                ...
                abstract_texts = []
                abstract_div = soup.find('section', id='author-abstract')
                if abstract_div:
                    paragraphs = abstract_div.find_all('div',role='paragraph')
...
async def crawl_article_analgesia(specialization):
    journal_name ='Anesthesia & Analgesia'
    connection = connection_config()
    table_name = specialization
...
async def crawl_article_clinical_anesthesia(specialization):
    journal_name = "Journal of Clinical Anesthesia"
    connection = connection_config()
    table_name = specialization
    ...
    try:
        ...
        for link_tuple in links:
            ...
            try:
                ...
                article_title = meta_tags.get("citation_title", "N/A")
                article_journal_title = 'Journal of Clinical Anesthesia'
                article_language = meta_tags.get("citation_language", "English")
                article_volume = meta_tags.get("citation_volume", "N/A")
                article_issue = meta_tags.get("citation_issue", "N/A")
                article_publication_date = meta_tags.get("citation_publication_date", "2001/01/01")
                article_publisher = meta_tags.get('citation_publisher', 'N/A')
                article_full_pdf_link = article_url+'/pdf'
                article_doi = meta_tags.get("citation_doi", "N/A")
                article_issn = meta_tags.get("citation_issn", "N/A")
                ...
                if author_group:
                    for author in author_group.find_all("span", class_="react-xocs-alternative-link"):
                        given_name = author.find("span", class_="given-name")
                        surname = author.find("span", class_="text surname")
                        if given_name and surname:
                            authors.append(f"{given_name.get_text(strip=True)} {surname.get_text(strip=True)}")
                article_author = ', '.join(authors) if authors else "N/A"
                article_abstract=None
                abstract_div = soup.find("div", class_="abstract author")
                if abstract_div:
                    article_abstract=abstract_div.get_text(strip=True, separator=" ")
                else:
...
async def crawl_article_rapm(specialization):
    journal_name = "Regional Anesthesia & Pain Medicine"
    connection = connection_config()
    table_name = specialization
...
async def main():
    specialization='Anesthesiology'
    await crawl_article_anesthesiology(specialization)
