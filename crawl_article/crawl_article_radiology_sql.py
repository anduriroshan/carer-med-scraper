...
async def crawl_article_radiology(specialization):
    ...
    try:
        ...
        for link_tuple in links:
            ...
            try:
                ...
                article_doi = soup.find("meta", attrs={"name":'dc.Identifier','scheme':'doi'}).get("content")
                article_issn = meta_tags.get("citation_issn", "N/A")
                article_volume = "N/A"
                article_issue = "N/A"

                # Find all <div> elements with class "meta"
                meta_divs = soup.find_all('div', class_='meta')

                for meta_div in meta_divs:
                    # Check for <a> tag inside <div>
                    a_tag = meta_div.find('a')
                    if a_tag and "Vol." in a_tag.text:
                        match = re.search(r'Vol\.\s*(\d+),\s*No\.\s*(\d+)', a_tag.text)
                        if match:
                            article_volume, article_issue = match.groups()
                            break  # Exit loop if found

                    # Check for volume/issue in the plain text of <div>
                    elif "Vol." in meta_div.text:
                        match = re.search(r'Vol\.\s*(\d+),\s*No\.\s*(\d+)', meta_div.text)
                        if match:
                            article_volume, article_issue = match.groups()
                            break  # Exit loop if found

                authors_list = []
                authors_ul = soup.find('ul', class_='rlist--inline loa hidden-xs loa-accordion')

                if authors_ul:
                    author_links = authors_ul.find_all('a', title=True)
                    authors_list = [a['title'] for a in author_links]

                article_author = ', '.join(authors_list) if authors_list else "N/A"
...
async def crawl_article_european_radiology(specialization):
    journal_name='European Radiology'
    connection = connection_config()
    table_name = specialization
        ...
        for link_tuple in links:
            ...
            try:
                ...
                article_publication_date = meta_tags.get(
                    "citation_online_date", "2001/01/01"
                )
...
async def crawl_article_arj(specialization):
    journal_name='American Journal of Roentgenology'
    connection = connection_config()
    table_name = specialization
...
async def crawl_article_magnetic_resonance(specialization):
    journal_name = 'Journal of Magnetic Resonance Imaging'
    connection = connection_config()
    table_name = specialization
...
async def crawl_article_investigative_radiology(specialization):
    journal_name ='Investigative Radiology'
    connection = connection_config()
    table_name = specialization
