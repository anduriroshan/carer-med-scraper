...
###################################################################################################################################################################################################
def process_and_store_embeddings(article_title, article_abstract, article_author, article_url, table_name, cursor, connection, specialization):
    """
    Generates embeddings, stores data in Milvus, and updates the database.
    """
    try:
        # Generate embeddings (ensure flat lists)
        ...
        # Prepare metadata with specialization
        article_metadata = {
            ...
            "title_embedding": title_embedding,  # Should be flat list
            "abstract_embedding": abstract_embedding,  # Should be flat list
            "authors_embedding": authors_embedding,  # Should be flat list
        }

        # Insert into specialization-specific collection
        milvus_collection = initialize_milvus(specialization)
        add_to_milvus(milvus_collection, article_metadata)

        #Insert into merged collection
        merged_collection = initialize_milvus_merged('merged')
        add_to_milvus_merged(merged_collection, article_metadata)

        # Update SQL database to mark embeddings as 'done'
        update_query = f"UPDATE {table_name} SET embeddings = 'done' WHERE article_url = %s"
        cursor.execute(update_query, (article_url,))
        connection.commit()
        print(f"Successfully processed and updated {specialization} table: {article_url}\n")
...
def add_to_milvus(collection, article_metadata):
    ...
    """
    # Prepare the data for each field in the schema (ensure each is a separate list)
    data_to_insert = [
        [article_metadata["title_text"]],  # Title text (list)
        [article_metadata["abstract_text"]],  # Abstract text (list)
        [article_metadata["authors_text"]],  # Authors text (list)
        [article_metadata["article_url"]],  # Article URL (list)
        article_metadata["title_embedding"],  # Title embedding (flattened list)
        article_metadata["abstract_embedding"],  # Abstract embedding (flattened list)
        article_metadata["authors_embedding"],  # Authors embedding (flattened list)
    ]
...
def add_to_milvus_merged(collection, article_metadata):
    """
    Add article metadata to Milvus collection.
    ...
    data_to_insert = [
        ...
        [article_metadata["specialization"]], # Article URL (list)
        article_metadata["title_embedding"],  # Title embedding (flattened list)
        article_metadata["abstract_embedding"],  # Abstract embedding (flattened list)
        article_metadata["authors_embedding"],  # Authors embedding (flattened list)
    ]
...
def create_milvus_collection(collection_name):
    ...
    fields = [
        FieldSchema(name="id", dtype=DataType.INT64, is_primary=True, auto_id=True),  # Auto ID field
        FieldSchema(name="title_text", dtype=DataType.VARCHAR, max_length=65535),  # Title text field
        FieldSchema(name="abstract_text", dtype=DataType.VARCHAR, max_length=65535),  # Abstract text field
        FieldSchema(name="authors_text", dtype=DataType.VARCHAR, max_length=65535),  # Authors text field
        FieldSchema(name="article_url", dtype=DataType.VARCHAR, max_length=65535),  # Article URL field
        FieldSchema(name="title_embedding", dtype=DataType.FLOAT_VECTOR, dim=768),  # Embedding for title
        FieldSchema(name="abstract_embedding", dtype=DataType.FLOAT_VECTOR, dim=768),  # Embedding for abstract
        FieldSchema(name="authors_embedding", dtype=DataType.FLOAT_VECTOR, dim=768),  # Embedding for authors
    ]
...
def create_milvus_collection_merged(collection_name):
    ...
    # Define the schema with an INT64 primary key
    fields = [
    ...
    schema = CollectionSchema(fields=fields, description="Merged collection of all specializations")

    # Create the collection
    Collection(name=collection_name, schema=schema)
    print(f"Collection '{collection_name}' created successfully with a primary key.")
